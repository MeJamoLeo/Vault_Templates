以下の指示に従って〇〇ゲートのドキュメントを作成してください。
実装はには以下のゲートコンポーネントを使ってください.
- Nand
- Not
- And
- Or
...
- Specificationセクション
	...
	- exampleという折りたためるブロックを作成し、その中にはMarmeidを用いたグラフを作成してください。
		- Marmeidで作成するグラフの数はTruth Tableを参考に、全てのパターンを作成してください。
		...
		- ノードからの矢印の上に受け渡しされる値を書き込んでください。
			- 値によって色分けしてください。
				- 1の場合は#00aa00
				- 0の場合は#aa0000
		- 出力のノードは値によって色分けしてください
			- 1の場合は99ff99
			- 0の場合はff9999
		- classDef geteを設定し、ゲートコンポーネントにあたるノードは白い枠線を使ってください。
- Implementationセクション
	- tipというブロックを作成し、その中には論理式を書いてください。
		- Truth Tableを参考に出力が真のものを\lorで繋ぐところから始めてください。
		- 途中の計算過程の論理式を1ステップずつ書いてください。
		...
		- 実装に基づいたMarmeidをベースとして以下を作成してください。
			- exampleという折りたためるブロックを作成し、その中にはMarmeidを用いたグラフを作成してください。
				...
				- 出力のノードは値によって色分けしてください
					- 1の場合は99ff99
					- 0の場合はff9999
		- 最適化の手順をproveというブロックを使って説明してください
- Mermaidで使うNordの中はAnd, Not, Nandのようにゲート名を使ってください

===

以下はXorを例にしたドキュメントのサンプルです。
